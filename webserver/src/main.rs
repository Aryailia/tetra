...
fn handle_client(public: &str, mut stream: TcpStream) -> io::Result<()> {
    // linux/limits.h max path is 4096
    let mut buffer = [0; 4096 + "GET HTTP".len()]; // Big enough to hold first line
...
define_str! {
    NOT_FOUND = {
        NOT_FOUND_HEAD = "HTTP/1.1 404 Not Found\r\n\r\n"
        _NOT_FOUND_BODY = "<h1>404 File Not Found.</h1>"
    }
    _FORBIDDEN = {
        _FORBIDDEN_HEAD = "HTTP/1.1 403 FORBIDDEN\r\n\r\n"
        _FORBIDDEN_BODY = "<h1>403 Forbidden.</h1>"
    }
    _OK = {
...
fn serve_static_file(public_root: &str, request: &str) -> String {
    let paths = [
        ([public_root, request, "/index.html"].join(""), OK_HEAD),
        ([public_root, request].join(""), OK_HEAD),
        ([public_root, "/404.html"].join(""), NOT_FOUND_HEAD),
    ];
